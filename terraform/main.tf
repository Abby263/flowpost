...
data "azurerm_key_vault_secrets" "optional" {
  count        = var.enable_key_vault && var.preserve_optional_secrets ? 1 : 0
  key_vault_id = module.key_vault[0].key_vault_id
}

data "azurerm_key_vault_secret" "openai_api_key" {
  count        = var.enable_key_vault && var.preserve_optional_secrets && !local.openai_api_key_provided && local.openai_api_key_exists ? 1 : 0
  name         = "openai-api-key"
  key_vault_id = module.key_vault[0].key_vault_id
}

data "azurerm_key_vault_secret" "gemini_api_key" {
  count        = var.enable_key_vault && var.preserve_optional_secrets && !local.gemini_api_key_provided && local.gemini_api_key_exists ? 1 : 0
  name         = "gemini-api-key"
  key_vault_id = module.key_vault[0].key_vault_id
}

data "azurerm_key_vault_secret" "langchain_api_key" {
  count        = var.enable_key_vault && var.preserve_optional_secrets && !local.langchain_api_key_provided && local.langchain_api_key_exists ? 1 : 0
  name         = "langchain-api-key"
  key_vault_id = module.key_vault[0].key_vault_id
}

data "azurerm_key_vault_secret" "serper_api_key" {
  count        = var.enable_key_vault && var.preserve_optional_secrets && !local.serper_api_key_provided && local.serper_api_key_exists ? 1 : 0
  name         = "serper-api-key"
  key_vault_id = module.key_vault[0].key_vault_id
}

data "azurerm_key_vault_secret" "perplexity_api_key" {
  count        = var.enable_key_vault && var.preserve_optional_secrets && !local.perplexity_api_key_provided && local.perplexity_api_key_exists ? 1 : 0
  name         = "perplexity-api-key"
  key_vault_id = module.key_vault[0].key_vault_id
}

data "azurerm_key_vault_secret" "firecrawl_api_key" {
  count        = var.enable_key_vault && var.preserve_optional_secrets && !local.firecrawl_api_key_provided && local.firecrawl_api_key_exists ? 1 : 0
  name         = "firecrawl-api-key"
  key_vault_id = module.key_vault[0].key_vault_id
}

data "azurerm_key_vault_secret" "stripe_secret_key" {
  count        = var.enable_key_vault && var.preserve_optional_secrets && !local.stripe_secret_key_provided && local.stripe_secret_key_exists ? 1 : 0
  name         = "stripe-secret-key"
  key_vault_id = module.key_vault[0].key_vault_id
}

data "azurerm_key_vault_secret" "stripe_webhook_secret" {
  count        = var.enable_key_vault && var.preserve_optional_secrets && !local.stripe_webhook_secret_provided && local.stripe_webhook_secret_exists ? 1 : 0
  name         = "stripe-webhook-secret"
  key_vault_id = module.key_vault[0].key_vault_id
}

data "azurerm_key_vault_secret" "admin_user_ids" {
  count        = var.enable_key_vault && var.preserve_optional_secrets && !local.admin_user_ids_provided && local.admin_user_ids_exists ? 1 : 0
  name         = "admin-user-ids"
  key_vault_id = module.key_vault[0].key_vault_id
}
