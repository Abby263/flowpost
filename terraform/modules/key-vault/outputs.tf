...
output "openai_api_key_secret_uri" {
  ...
  value       = local.openai_api_key_value_present ? "${azurerm_key_vault.this.vault_uri}secrets/openai-api-key" : null
}

output "gemini_api_key_secret_uri" {
  description = "URI for Gemini API key secret"
  value       = local.gemini_api_key_value_present ? "${azurerm_key_vault.this.vault_uri}secrets/gemini-api-key" : null
}

output "langchain_api_key_secret_uri" {
  description = "URI for LangChain API key secret"
  value       = local.langchain_api_key_value_present ? "${azurerm_key_vault.this.vault_uri}secrets/langchain-api-key" : null
}

output "serper_api_key_secret_uri" {
  description = "URI for Serper API key secret"
  value       = local.serper_api_key_value_present ? "${azurerm_key_vault.this.vault_uri}secrets/serper-api-key" : null
}

output "perplexity_api_key_secret_uri" {
  description = "URI for Perplexity API key secret"
  value       = local.perplexity_api_key_value_present ? "${azurerm_key_vault.this.vault_uri}secrets/perplexity-api-key" : null
}

output "firecrawl_api_key_secret_uri" {
  description = "URI for FireCrawl API key secret"
  value       = local.firecrawl_api_key_value_present ? "${azurerm_key_vault.this.vault_uri}secrets/firecrawl-api-key" : null
}

output "clerk_publishable_key_secret_uri" {
  description = "URI for Clerk publishable key secret"
  value       = local.clerk_publishable_key_present ? "${azurerm_key_vault.this.vault_uri}secrets/clerk-publishable-key" : null
}

output "clerk_secret_key_secret_uri" {
  description = "URI for Clerk secret key secret"
  value       = local.clerk_secret_key_present ? "${azurerm_key_vault.this.vault_uri}secrets/clerk-secret-key" : null
}

output "stripe_secret_key_secret_uri" {
  description = "URI for Stripe secret key secret"
  value       = local.stripe_secret_key_value_present ? "${azurerm_key_vault.this.vault_uri}secrets/stripe-secret-key" : null
}

output "stripe_webhook_secret_secret_uri" {
  description = "URI for Stripe webhook secret"
  value       = local.stripe_webhook_secret_value_present ? "${azurerm_key_vault.this.vault_uri}secrets/stripe-webhook-secret" : null
}

output "admin_user_ids_secret_uri" {
  description = "URI for admin user IDs secret"
  value       = local.admin_user_ids_value_present ? "${azurerm_key_vault.this.vault_uri}secrets/admin-user-ids" : null
}

output "acr_password_secret_uri" {
  description = "URI for ACR password secret"
  value       = local.acr_password_present ? "${azurerm_key_vault.this.vault_uri}secrets/acr-password" : null
}
