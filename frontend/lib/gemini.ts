...
export async function generateImageWithGemini(
  ...
  if (
    ...
    console.warn(
      `⚠️  [Gemini] Invalid model name "${model}", defaulting to gemini-2.5-flash-image`,
    );
...
> {
  ...
  if (
    ...
    console.log(
      `⚠️  [Gemini] Overriding IMAGE_MODEL (${model}) to gemini-3-pro-image-preview for Google Search grounding`,
    );
    model = "gemini-3-pro-image-preview";
  }
  ...
  try {
    ...
    console.error(
      `❌ [Gemini Image Grounding] Error with model "${model}":`,
      error.message,
    );
    ...
    throw new Error(
      `Gemini image generation with grounding failed (model: ${model}): ${error.message}`,
    );
  }
}
