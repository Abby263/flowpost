...
async function getGeneralContent(url: string): Promise<{
  ...
  const isProtectedPlatform = /instagram\.com|facebook\.com|linkedin\.com/i.test(url);
  
  if (isProtectedPlatform) {
    console.log(`⚠️  Protected platform detected (${url}), attempting scrape with caution...`);
  }
  ...
  try {
    ...
    console.warn(`❌ FireCrawl failed for ${url}: ${errorMsg}`);
    throw new Error(`Failed to fetch content from ${url}: ${errorMsg}`);
  }
}
...
async function getTwitterContent(url: string): Promise<{
  ...
  const externalUrlPromises = externalUrls.map(async (url) => {
    ...
    try {
      ...
      console.error(`❌ Failed to get content from ${url} extracted in Tweet: ${e.message}`);
    }
