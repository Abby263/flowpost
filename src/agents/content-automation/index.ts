...
async function fetchContent(state: typeof ContentAutomationState.State) {
    console.log(`Fetching content for query: ${state.searchQuery} in ${state.location}`);
    ...
    if (!SERPER_API_KEY) {
        ...
        return {
            events: [
                { title: "Mock Event 1", date: "Tomorrow", link: "https://example.com/1" },
                { title: "Mock Event 2", date: "Weekend", link: "https://example.com/2" },
            ]
        };
    }
...
async function curateContent(state: typeof ContentAutomationState.State) {
  ...
  `;
    ...
    try {
        ...
        const selectedContent = jsonMatch ? JSON.parse(jsonMatch[0]) : state.events.slice(0, 3);
...
async function generateVisuals(state: typeof ContentAutomationState.State) {
    ...
    try {
        ...
        const { getPlaceholderImageUrl } = await import("../../utils/image-generation.js");
