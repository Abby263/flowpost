...
const ContentAutomationAdvancedState = Annotation.Root({
    ...
    relevantLinks: Annotation<string[] | undefined>({
        reducer: (x, y) => y, // Simplified reducer for now
        default: () => [],
    }),
...
async function fetchContent(state: typeof ContentAutomationAdvancedState.State) {
    ...
    if (!SERPER_API_KEY) {
        console.warn("⚠️  [FETCH CONTENT] SERPER_API_KEY not set. Using mock data.");
        return {
            events: [
                { title: "Mock Event 1", date: "Tomorrow", link: "https://example.com/1", snippet: "This is mock data because Serper API key is not configured." },
                { title: "Mock Event 2", date: "Weekend", link: "https://example.com/2", snippet: "Please add SERPER_API_KEY to your .env file." },
            ],
    ...
    try {
        ...
        console.log(`✅ [FETCH CONTENT] Successfully fetched ${events.length} results`);
...
async function curateContent(state: typeof ContentAutomationAdvancedState.State) {
    console.log(`\n🎯 [CURATE CONTENT] Starting content curation...`);
    console.log(`   Style: "${state.stylePrompt || 'Professional and engaging'}"`);
    console.log(`   Input events: ${state.events.length} items`);
    
    if (!state.events || state.events.length === 0) {
        console.warn("⚠️  [CURATE CONTENT] No events to curate. Using empty selection.");
        return {
            selectedContent: [],
            links: [],
            relevantLinks: [],
  ...
  `;
    ...
    try {
        ...
        if (useGemini && process.env.GEMINI_API_KEY) {
            console.log(`   🤖 Calling Gemini (${process.env.LLM_MODEL || "gemini-2.0-flash-exp"}) for curation...`);
            ...
            console.log(`   🤖 Calling OpenAI (${process.env.LLM_MODEL || "gpt-4o-mini"}) for curation...`);
        ...
        console.log(`✅ [CURATE CONTENT] Successfully curated ${selected.length} items`);
...
async function generateVisuals(state: typeof ContentAutomationAdvancedState.State) {
    ...
    try {
        ...
        if (result.imageUrl && result.imageUrl.startsWith('data:')) {
            console.log(`   URL: [Base64 data URL - ${result.imageUrl.length} chars]`);
        ...
        return {
            ...
            apiCosts: { imageGeneration: result.cost || 0.040 }
        };
    } catch (error: any) {
...
async function checkContentQuality(state: typeof ContentAutomationAdvancedState.State) {
    console.log(`\n✅ [CHECK QUALITY] Evaluating content quality...`);
    
    if (!state.report || state.report.length < 50) {
        console.warn("⚠️  [CHECK QUALITY] Report is too short or empty. Marking as insufficient.");
        return {
    ...
    try {
        ...
        if (useGemini && process.env.GEMINI_API_KEY) {
            console.log(`   🤖 Asking Gemini (${process.env.LLM_MODEL || "gemini-2.0-flash-exp"}) to evaluate quality...`);
            const { generateTextWithGemini } = await import("../../utils/gemini.js");
            content = await generateTextWithGemini(prompt, undefined, {
                temperature: 0,
            });
            ...
            console.log(`   🤖 Asking OpenAI (${process.env.LLM_MODEL || "gpt-4o-mini"}) to evaluate quality...`);
            const response = await llm.invoke([new HumanMessage(prompt)]);
            const duration = Date.now() - startTime;
            console.log(`   ⏱️  OpenAI response time: ${duration}ms`);
            content = response.content as string;
        ...
        if (!jsonMatch) {
            console.warn("⚠️  [CHECK QUALITY] No JSON found in response. Assuming sufficient.");
            return { isContentSufficient: true, feedback: "Could not parse quality check" };
        }
        
        const result = JSON.parse(jsonMatch[0]);
        console.log(`   ${result.sufficient ? '✅' : '❌'} Quality check result: ${result.sufficient ? 'SUFFICIENT' : 'INSUFFICIENT'}`);
        ...
        return { isContentSufficient: true, feedback: "Error in quality check, proceeding anyway" };
    }
}

function routeContentQuality(state: typeof ContentAutomationAdvancedState.State) {
...
async function publishToInstagram(state: typeof ContentAutomationAdvancedState.State) {
    console.log(`--- PUBLISHING TO ${state.platform?.toUpperCase()} ---`);
    
    // Check if there was a previous failure
    if (state.publishStatus === "failed") {
        console.error(`❌ Skipping publish - previous error: ${state.publishError}`);
    ...
    if (state.platform !== "instagram") {
        console.log(`⚠️  Platform ${state.platform} not yet implemented in this workflow.`);
        return { publishStatus: "skipped", publishError: `Platform ${state.platform} not supported` };
    }
    ...
    try {
        ...
        console.log(`   Published URL: https://www.instagram.com/p/${result.media?.code || 'N/A'}/`);
        return { 
        ...
        if (error.message?.includes('login_required') || error.message?.includes('IgLoginRequiredError')) {
            userFriendlyError = `Instagram login failed. This could be due to:\n` +
        ...
        } else if (error.message?.includes('verification') || error.message?.includes('challenge')) {
            userFriendlyError = `Instagram requires account verification. ` +
...
async function savePostToDb(state: typeof ContentAutomationAdvancedState.State) {
    ...
    if (!supabaseUrl || !supabaseKey) {
        ...
        console.warn("   Set either NEXT_PUBLIC_SUPABASE_URL + NEXT_PUBLIC_SUPABASE_ANON_KEY");
...
async function prepareCaption(state: typeof ContentAutomationAdvancedState.State) {
    ...
    if (state.post && state.post.length > 10) {
        console.log(`✅ [PREPARE CAPTION] Using existing post (${state.post.length} chars)`);
    ...
    if (state.report) {
        ...
        const lines = state.report.split('\n').filter(l => l.trim().length > 0).slice(0, 5);
        caption += lines.join('\n');
    } else {
...
function routePublishing(state: typeof ContentAutomationAdvancedState.State): "publishToInstagram" | "savePostToDb" {
...
export const contentAutomationAdvancedGraph = new StateGraph(ContentAutomationAdvancedState)
