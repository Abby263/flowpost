  ...
  if (state.platform === "instagram") {
    ...
    try {
      ...
      if (state.credentials) {
        const { apiKey, apiKeySecret, accessToken, accessTokenSecret } = state.credentials;
    ...
    try {
      ...
      if (state.credentials) {
        ...
        linkedInClient = await LinkedInClient.fromArcade(linkedInUserId, { postToOrganization: postToLinkedInOrg });
      } else {
        // Env fallback
        const accessToken = process.env.LINKEDIN_ACCESS_TOKEN || config.configurable?.[LINKEDIN_ACCESS_TOKEN];
        if (!accessToken) throw new Error("LinkedIn access token not found");
        linkedInClient = new LinkedInClient({
          accessToken,
          personUrn: process.env.LINKEDIN_PERSON_URN || config.configurable?.[LINKEDIN_PERSON_URN],
          organizationId: process.env.LINKEDIN_ORGANIZATION_ID || config.configurable?.[LINKEDIN_ORGANIZATION_ID],
        });
      }
