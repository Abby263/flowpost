...
export function getAIProvider(): "openai" | "gemini" {
  ...
  if (aiProvider === "gemini" || isGeminiModel(llmModel)) {
    if (!isGeminiConfigured()) {
      console.warn("⚠️  Gemini selected but GEMINI_API_KEY not configured. Falling back to OpenAI.");
      return "openai";
    }
    return "gemini";
  }
...
function getOpenAIModel(config: LLMConfig = {}): ChatOpenAI {
  return new ChatOpenAI({
    modelName: config.modelName || config.model || process.env.LLM_MODEL || "gpt-4o",
    temperature: config.temperature ?? parseFloat(process.env.LLM_TEMPERATURE || "0.5"),
...
async function generateWithGemini(
  ...
  const model = config.modelName || config.model || process.env.LLM_MODEL || "gemini-2.0-flash-exp";

  // Use Google Search grounding if enabled and requested
  if (config.useGrounding && process.env.USE_GOOGLE_SEARCH_GROUNDING === "true") {
          ...
          try {
            ...
            if (shape) {
              ...
              for (const [key, value] of Object.entries(shape)) {
                ...
                fields.push(`"${key}": ${fieldType}${description ? ` (${description})` : ""}`);
              }
        ...
        try {
          ...
          try {
            ...
            if (jsonMatch) {
              ...
              const reasoningMatch = jsonStr.match(/(?:reasoning|Reasoning):\s*["\']?([\s\S]+?)(?=\n(?:relevant|is_relevant|Is the content relevant)|$)/i);
              const relevantMatch = jsonStr.match(/(?:relevant|is_relevant|Is the content relevant[^:]*?):\s*(true|false)/i);
          ...
          if (parsed.is_relevant !== undefined && parsed.relevant === undefined) {
    ...
    return {
      async invoke(messages: BaseMessage[]): Promise<any> {
        ...
        try {
          ...
          console.warn("⚠️  Could not parse tool call from Gemini:", error.message);
          return {
            content: text,
            tool_calls: [],
          };
...
export async function generateText(
  ...
  if (provider === "gemini") {
    ...
    if (options?.useGrounding && process.env.USE_GOOGLE_SEARCH_GROUNDING === "true") {
      const result = await generateTextWithGrounding(prompt, options.systemPrompt, {
        model,
