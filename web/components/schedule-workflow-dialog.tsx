...
export function ScheduleWorkflowDialog({ isOpen, onClose, onSchedule, workflowName }: ScheduleWorkflowDialogProps) {
    const [selectedDate, setSelectedDate] = useState<Date | undefined>(new Date());
    const [selectedTime, setSelectedTime] = useState("09:00");
    const [repeatType, setRepeatType] = useState<"once" | "daily" | "weekly" | "monthly" | "yearly">("once");
                ...
                <div className="space-y-6 py-4">
                    ...
                    <div className="space-y-2">
                        ...
                        <div className="flex gap-2">
                            <Select value={selectedTime.split(':')[0]} onValueChange={(h) => setSelectedTime(`${h}:${selectedTime.split(':')[1]}`)}>
                            ...
                            <Select value={selectedTime.split(':')[1]} onValueChange={(m) => setSelectedTime(`${selectedTime.split(':')[0]}:${m}`)}>
                                <SelectTrigger className="w-[100px]">
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="flex flex-wrap gap-2">
                                {weekDays.map((day) => (
                                    ...
                                    >
                                        {selectedDays.includes(day) && <Check className="mr-1 h-3 w-3" />}
                                        {day.slice(0, 3)}
                                    </Badge>
                                ))}
                            </div>
                    ...
                    <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                        <p className="text-sm font-medium text-blue-900 mb-1">Schedule Summary</p>
