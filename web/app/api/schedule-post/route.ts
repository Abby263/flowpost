...
export async function POST(request: Request) {
    try {
        ...
        if (action === "schedule") {
            ...
            try {
                const post = await schedulePost(content, platform, connectionId, scheduledAt, userId, imageUrl, source);
        ...
        if (action === "post-now") {
            ...
            try {
                ...
                if (platform === "instagram" && !imageUrl) {
                    return NextResponse.json({
                        error: "Instagram posts require an image. Please add an image before posting."
                    }, { status: 400 });
                }
                ...
                try {
                    ...
                    while (runStatus.status !== "success" && runStatus.status !== "error" && attempts < maxAttempts) {
                        await new Promise(resolve => setTimeout(resolve, 1000));
                        runStatus = await langGraphClient.runs.get(thread.thread_id, run.run_id);
                        attempts++;
                    }
                    ...
                    if (runStatus.status === "success") {
                        ...
                        if (updateError) {
                            console.error(`[Schedule Post API] Failed to update post status:`, updateError);
                        } else {
                            console.log(`[Schedule Post API] Successfully updated post ${post.id} to published`);
                        }
                        ...
                        if ((runStatus as any).error) {
                            ...
                            if (errorStr.includes("aspect ratio")) {
                                errorMsg = "Image has incorrect aspect ratio for Instagram. Try regenerating with AI or use a square image (1:1 ratio).";
                            } else if (errorStr.includes("login") || errorStr.includes("authentication")) {
                                errorMsg = "Authentication failed. Please check your Instagram credentials.";
                            } else if (errorStr.includes("rate limit")) {
                                errorMsg = "Instagram rate limit exceeded. Please wait 10-15 minutes and try again.";
                            } else {
                        ...
                        return NextResponse.json({
                            ...
                            message: "Post is being processed. Check back shortly for status."
                        });
                    }
