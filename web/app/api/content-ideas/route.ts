...
const USE_GOOGLE_SEARCH_GROUNDING = process.env.USE_GOOGLE_SEARCH_GROUNDING === "true";
...
async function searchWithSerper(query: string): Promise<ContentItem[]> {
    ...
    console.log(`[Serper] Found ${results.length} recent results for query: ${query}`);
...
async function searchWithTavily(query: string): Promise<ContentItem[]> {
    ...
    console.log(`[Tavily] Found ${data.results?.length || 0} recent results for query: ${query}`);
    ...
    console.log(`[Tavily] Filtered to ${recentResults.length} results from last 7 days`);
...
async function searchWithPerplexity(query: string): Promise<ContentItem[]> {
    ...
    const response = await fetch("https://api.perplexity.ai/chat/completions", {
        ...
        body: JSON.stringify({
            ...
            messages: [
                { role: "system", content: "You are a helpful news assistant. Search for the latest and most recent news (from the last 7 days only) about the user's query. Return a list of 5-10 specific recent news items with titles, brief snippets, publication dates, and source URLs. Format the output as a JSON array of objects with keys: title, snippet, link, source, date." },
                { role: "user", content: `Find the most recent trending news from the last week about: ${query}` }
            ],
            temperature: 0.2,
        }),
    });

    if (!response.ok) throw new Error(`Perplexity API returned ${response.status}`);
    ...
    try {
        ...
        console.warn("Failed to parse Perplexity JSON response, returning raw text as one item", e);
    }
...
async function fetchTrendingContent(query: string, provider: string = "auto"): Promise<ContentItem[]> {
    let results: ContentItem[] = [];
    let errors: string[] = [];

    // Try specific provider if requested
    if (provider === "serper" && SERPER_API_KEY) {
        try { return await searchWithSerper(query); } catch (e: any) { errors.push(`Serper: ${e.message}`); }
    }
    if (provider === "tavily" && TAVILY_API_KEY) {
        try { return await searchWithTavily(query); } catch (e: any) { errors.push(`Tavily: ${e.message}`); }
    }
    if (provider === "perplexity" && PERPLEXITY_API_KEY) {
        try { return await searchWithPerplexity(query); } catch (e: any) { errors.push(`Perplexity: ${e.message}`); }
    }
    ...
    return [
        {
            ...
            snippet: "Real search failed. Please configure SERPER_API_KEY, TAVILY_API_KEY, or PERPLEXITY_API_KEY.",
            link: "https://example.com/ai-news-1",
            source: "System"
        },
        {
            title: "Mock Data: Future of Work",
            snippet: "This is placeholder content because no search provider API keys are valid.",
 ...
 */
async function generatePostIdeasWithGemini(content: ContentItem[], category: string): Promise<GeneratedIdea[]> {
    try {
        const contentSummary = content.slice(0, 5).map(item =>
            `- ${item.title}: ${item.snippet}`
        ).join("\n");

        const systemPrompt = "You are a creative social media content strategist. You MUST respond with ONLY a valid JSON array, no other text before or after.";
...
[{"title":"Example Title","caption":"Example caption 💡","hashtags":["example","social"],"style":"Professional"}]`;
        ...
        if (USE_GOOGLE_SEARCH_GROUNDING) {
            console.log("🔍 [Gemini] Using Google Search grounding for real-time content ideas");
            ...
            if (groundingMetadata) {
                console.log("✅ [Gemini] Grounding metadata received:", JSON.stringify(groundingMetadata, null, 2));
            }
                        } else {
        ...
        try {
            ...
            throw new Error("Gemini response is not valid JSON. Response: " + cleanedResponse.substring(0, 500));
        }
        ...
        const ideasWithImages = await Promise.all(
            ideas.map(async (idea: GeneratedIdea) => {
                try {
                    ...
                    if (USE_GOOGLE_SEARCH_GROUNDING) {
                        try {
                            console.log(`🖼️  [Gemini] Generating image with Google Search grounding: "${idea.title}"`);
                            ...
                            if (images.length > 0) {
                                ...
                                console.log(`✅ [Gemini] Image generated successfully with grounding`);
                            }
                        } catch (imgError) {
                            console.warn(`⚠️  [Gemini] Image generation failed, using placeholder:`, imgError);
                        }
                    }
                    ...
                    console.warn("Failed to generate image for idea, using placeholder:", error);
                    return {
                        ...idea,
 ...
 */
async function generatePostIdeasWithOpenAI(content: ContentItem[], category: string): Promise<GeneratedIdea[]> {
    if (!OPENAI_API_KEY) {
...
Only return valid JSON, no markdown or additional text.`;
        ...
        const response = await fetch("https://api.openai.com/v1/chat/completions", {
            ...
            body: JSON.stringify({
                ...
                messages: [
                    { role: "system", content: "You are a creative social media content strategist. Always respond with valid JSON only." },
 ...
 */
async function generatePostIdeas(content: ContentItem[], category: string): Promise<GeneratedIdea[]> {
    ...
    try {
        ...
        if (action === "fetch") {
            ...
            if (query) {
                ...
                searchQueries = [query];
            } else if (interests && Array.isArray(interests) && interests.length > 0) {
                console.log(`[Content Ideas API] Using interests: ${interests.join(", ")}`);
                // Agentic Step: Generate smart search queries based on interests
                const useGemini = AI_PROVIDER === "gemini" || isGeminiModel();
                
                if (useGemini && isGeminiConfigured()) {
                    try {
                        console.log(`[Content Ideas API] Generating smart search queries with Gemini...`);
                        const systemPrompt = "You are a helpful assistant. Return valid JSON array only.";
...
Return ONLY a JSON array of strings. Example: ["latest AI tools 2024", "SpaceX Starship update", "React 19 features"]`;
                        ...
                        let responseText: string;
                        if (USE_GOOGLE_SEARCH_GROUNDING) {
                            const result = await generateTextWithGrounding(prompt, systemPrompt, { temperature: 0.7 });
                            responseText = result.text;
                        } else {
                            const { generateTextWithGemini } = await import("../../../lib/gemini");
                            responseText = await generateTextWithGemini(prompt, systemPrompt, { temperature: 0.7 });
                        }

                        const jsonStr = responseText.replace(/```json\n?|```/g, "").trim();
                        searchQueries = JSON.parse(jsonStr);
                        console.log(`[Content Ideas API] Generated queries with Gemini: ${searchQueries.join(", ")}`);
                    } catch (e) {
                        console.error("Failed to generate smart queries with Gemini, falling back to raw interests", e);
                        searchQueries = interests;
                    }
                } else if (OPENAI_API_KEY) {
                    try {
                        console.log(`[Content Ideas API] Generating smart search queries with OpenAI...`);
                        const prompt = `You are a trend spotter. The user is interested in: ${interests.join(", ")}.
Generate 3 specific, trending search queries to find the latest news and content for them.
Return ONLY a JSON array of strings. Example: ["latest AI tools 2024", "SpaceX Starship update", "React 19 features"]`;
                        ...
                        const response = await fetch("https://api.openai.com/v1/chat/completions", {
                            ...
                            body: JSON.stringify({
                                model: process.env.LLM_MODEL || "gpt-4o-mini",
                                messages: [
                                    { role: "system", content: "You are a helpful assistant. Return valid JSON array only." },
                                    { role: "user", content: prompt }
                                ],
                                temperature: 0.7,
                            })
                        ...
                        searchQueries = JSON.parse(jsonStr);
                        console.log(`[Content Ideas API] Generated queries with OpenAI: ${searchQueries.join(", ")}`);
                    } catch (e) {
                        console.error("Failed to generate smart queries with OpenAI, falling back to raw interests", e);
                        searchQueries = interests;
                    }
                } else {
                    console.log(`[Content Ideas API] No AI provider configured, using raw interests as queries`);
                    searchQueries = interests;
                }
            } else {
                return NextResponse.json({ error: "Query or interests are required" }, { status: 400 });
            }
            ...
            console.log(`[Content Ideas API] Executing ${queriesToRun.length} searches (${RESULTS_PER_QUERY} results each): ${queriesToRun.join(", ")}`);
            const resultsPromises = queriesToRun.map(q => fetchTrendingContent(q, provider));
            ...
            console.log(`[Content Ideas API] Returning ${uniqueContent.length} unique results`);

            return NextResponse.json({ success: true, content: uniqueContent, queries: queriesToRun });
        }
        ...
        if (action === "generate") {
            ...
            const context = (interests && interests.length > 0) ? interests.join(", ") : (category || "trending topics");
            console.log(`[Content Ideas API] Generating post ideas for context: ${context} from ${content.length} articles`);
