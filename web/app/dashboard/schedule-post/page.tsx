...
export default function SchedulePostPage() {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (data.success) {
                ...
                const successMessage = data.message || (isInstantPost
                ...
                showNotification("error", data.error || `Failed to ${isInstantPost ? 'publish' : 'schedule'} post`);
            }
        } catch (error) {
            showNotification("error", `Failed to ${isInstantPost ? 'publish' : 'schedule'} post`);
        } finally {
            setSubmitting(false);
        }
    };
    ...
    const filteredConnections = connections.filter(c => !platform || c.platform === platform);
        ...
        <div className="space-y-8 px-6 py-6 max-w-[1400px]">
            ...
            {notification && (
                <div className="fixed top-4 right-4 z-50 animate-in slide-in-from-top-2 duration-300">
                    <div
                        className={`flex items-center space-x-3 rounded-lg border-2 px-4 py-3 shadow-lg backdrop-blur-sm ${notification.type === "success"
            ...
            <div>
                ...
                <p className="text-muted-foreground mt-2">
                    Create and schedule posts manually when you already have your content ready.
                </p>
            </div>
                        ...
                        <CardContent className="space-y-4">
                            ...
                            <div className="space-y-2">
                                <div className="flex items-center justify-between">
                                    ...
                                    <span className={`text-xs ${content.length > getCharacterLimit() ? 'text-red-500' : 'text-muted-foreground'}`}>
                    ...
                    <Card>
                        <CardHeader>
                            <CardTitle className="flex items-center gap-2">
                                ...
                                Image {platform === "instagram" && <span className="text-pink-600">(Required)</span>}
                            </CardTitle>
                            <CardDescription>
                                {platform === "instagram"
                            ...
                            <div className="space-y-3 pt-2">
                                ...
                                {selectedDate ? (
                                    <Button
                                        ...
                                        disabled={submitting || !content || !platform || !connectionId}
                                    >
                                        {submitting ? (
                                            <>
                            ...
                            {scheduledPosts.length === 0 ? (
                                ...
                                <div className="space-y-3 max-h-[300px] overflow-y-auto pr-1">
                                    {scheduledPosts.map((post) => (
                                        <div key={post.id} className="flex items-start gap-3 p-3 rounded-lg bg-muted/50 hover:bg-muted transition-colors border">
                                        ...
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                ...
                                                >
                                                    ...
                                                    {post.status === "posting" && <Loader2 className="mr-1 h-3 w-3 animate-spin" />}
                                                    {post.status}
                                                </Badge>
                                                <p className="text-xs text-muted-foreground mt-1">
                                                    {post.posted_at && new Date(post.posted_at).getFullYear() > 1970
