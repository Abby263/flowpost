...
function getImageProvider(): "openai" | "gemini" {
  ...
  if (aiProvider === "gemini" || isGeminiImageModel(imageModel)) {
    if (!isGeminiConfigured()) {
      console.warn(
        "⚠️  Gemini selected for images but GEMINI_API_KEY not configured. Falling back to OpenAI.",
      );
      return "openai";
    }
    return "gemini";
...
async function generateImageWithOpenAI(
  ...
  console.log(`🖼️  [OpenAI] Generating image with ${imageModel}`);
  ...
  console.log(`✅ [OpenAI] Image generated in ${duration}ms`);
...
async function generateImageWithGeminiProvider(
  ...
  console.log(`🖼️  [Gemini] Generating image with ${imageModel}`);
  ...
  const duration = Date.now() - startTime;
  console.log(`✅ [Gemini] Image generated in ${duration}ms`);
...
export async function generateImage(
  ...
  try {
    ...
    const duration = Date.now() - startTime;
    console.log(`✅ [Image Generation] Total time: ${duration}ms`);

    return result;
  } catch (error: any) {
    const duration = Date.now() - startTime;
    console.error(
      `❌ [Image Generation] Failed after ${duration}ms:`,
      error.message,
    );
    throw error;
  }
 ...
 */
export function getPlaceholderImageUrl(text = "Content+Automation"): string {
