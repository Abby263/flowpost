...
function parseGeneration(generation: string): string {
  ...
  if (!reportMatch) {
    console.warn(
      "⚠️  Could not find <report> tags in generation, using raw output",
    );
    // Remove <thinking> tags and their content
    const withoutThinking = generation
...
export async function generateContentReport(
  ...
  const validPageContents = state.pageContents?.filter(
    (content) => content?.trim().length > 0,
  );

  if (!validPageContents?.length) {
    console.warn(
      "No valid page contents found to generate report. Using existing report or default.",
    );
    return {
      report:
        state.report ||
        "No detailed content available to generate report. Using initial summaries.",
    };
  }
