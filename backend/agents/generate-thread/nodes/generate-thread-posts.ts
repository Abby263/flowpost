...
export async function generateThreadPosts(
  ...
  const firstPostContent = await generateText(formattedFirstPostPrompt);
  const firstPost = parseTweetGeneration(firstPostContent);
  ...
  for (let i = 0; i < state.totalPosts - 2; i += 1) {
    ...
    const bodyPostContent = await generateText(formattedFollowingPostPrompt);
    bodyPosts.push(parseTweetGeneration(bodyPostContent));
  }
  ...
  const finalPostContent = await generateText(formattedFinalPostPrompt);
  const finalPost = parseTweetGeneration(finalPostContent);
