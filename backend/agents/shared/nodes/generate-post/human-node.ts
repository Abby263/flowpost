...
>(state: State, config: LangGraphRunnableConfig): Promise<Update> {
  ...
  // If approval is not required, automatically schedule the post
  if (state.requiresApproval === false) {
    console.log("Auto-approving post as requiresApproval is false");
    return {
      next: "schedulePost",
      scheduleDate: defaultDate,
      ...(state.post ? { post: state.post } : {}),
      ...(state.complexPost ? { complexPost: state.complexPost } : {}),
      image: state.image,
