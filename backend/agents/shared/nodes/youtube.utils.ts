...
function getYouTubeClientFromUrl(): youtube_v3.Youtube {
  ...
  if (!rawCredentials) {
    throw new Error(
      "GOOGLE_VERTEX_AI_WEB_CREDENTIALS is not set. YouTube features require Google credentials.",
    );
  }

  const trimmed = rawCredentials.trim();
  let parsedGoogleCredentials;

  // Support both inline JSON and a file path to keep secrets out of git.
  if (trimmed.startsWith("{")) {
    try {
      parsedGoogleCredentials = JSON.parse(trimmed);
    } catch (error) {
      throw new Error(
        "Failed to parse GOOGLE_VERTEX_AI_WEB_CREDENTIALS as JSON. Provide valid service account JSON or a file path.",
      );
    }
  } else {
    try {
      const fileContents = fs.readFileSync(trimmed, "utf8");
      parsedGoogleCredentials = JSON.parse(fileContents);
    } catch (error) {
      throw new Error(
        `Failed to read or parse GOOGLE_VERTEX_AI_WEB_CREDENTIALS file at "${trimmed}". Ensure the path exists and contains valid service account JSON.`,
      );
    }
  }
